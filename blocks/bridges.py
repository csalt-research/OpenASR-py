...
class ZeroBridge(nn.Module):
    ...
    def forward(self, encoder_final):
        ...
        if self.dec_rnn_type == 'GRU':
            ...
            return (torch.zeros(self.dec_layers, batch_size, self.dec_rnn_size).to(device),)
        else:
            batch_size = encoder_final[0].size(1)
            return (torch.zeros(self.dec_layers, batch_size, self.dec_rnn_size).to(device), \
                    torch.zeros(self.dec_layers, batch_size, self.dec_rnn_size).to(device))
...
class DenseBridge(nn.Module):
    ...
    def forward(self, encoder_final):
        ...
        h_dec = self.linear_h(h_enc.transpose(0,1).contiguous().view(batch_size, -1))
        ...
        if self.dec_rnn_type == 'GRU':
            ...
            if len(encoder_final) == 1:
                c_dec = self.linear_c(h_enc.transpose(0,1).contiguous().view(batch_size, -1))
            else:
                c_enc = encoder_final[1]
                c_dec = self.linear_c(c_enc.transpose(0,1).contiguous().view(batch_size, -1))
